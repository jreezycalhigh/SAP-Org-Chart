...
<body>
    ...
    <div class="container">
        ...
        <div class="detail-panel" id="github-sync-panel" style="width: 420px; right: -440px;">
            ...
            <div class="detail-row">
                ...
                <input type="password" id="gh-pat" placeholder="ghp_..." class="form-input" oninput="saveSyncCredentials()"/>
            ...
            <div class="detail-row" style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                <div>
                    ...
                    <input type="text" id="gh-owner" value="jreezycalhigh" class="form-input" oninput="saveSyncCredentials()"/>
                </div>
                <div>
                    <div class="detail-label">Repository Name</div>
                    <input type="text" id="gh-repo" value="SAP-Org-Chart" class="form-input" oninput="saveSyncCredentials()"/>
                </div>
            </div>
            ...
            <div class="detail-row" style="display: grid; grid-template-columns: 1.2fr 0.8fr; gap: 12px;">
                <div>
                    ...
                    <input type="text" id="gh-path" value="SAP_Account_Priority_Outreach_Hub.html" class="form-input" oninput="saveSyncCredentials()"/>
                </div>
                <div>
                    <div class="detail-label">Branch</div>
                    <input type="text" id="gh-branch" value="main" class="form-input" oninput="saveSyncCredentials()"/>
                </div>
            </div>
        ...
        <div class="bob-footer">
            ...
            Made with IBM Bob &nbsp;|&nbsp; <a href="#" onclick="openGitHubSyncPanel(); return false;" style="color: var(--muted-color); text-decoration: underline;">Configure GitHub Connection</a>
    ...
    <script>
        ...
        function renderScheduleTab() {{
            ...
            // Determine current week number relative to START_DATE (Sunday Sept 20, 2026)
            const START_DATE = new Date("2026-09-20");

            const nowWS = getWeekStart(new Date());

            const startWS = getWeekStart(START_DATE);

            const currentWeekNum = Math.max(1, Math.floor(weeksBetween(startWS, nowWS)) + 1);
            ...
            function getContactWeekNum(c) {{
                ...
                if (s === "Past Due" || s === "Overdue") return -2;

                if (s === "This Week" || s === "Follow Up") return currentWeekNum;
            ...
            let filtered = matched.filter(c => {{
                ...
                if (viewFilter === "thisweek") {{
                    const isPastDue = (w === -2) || (w >= 1 && w < currentWeekNum && !contacted);
                    const isThisWeek = (w === currentWeekNum) || (s === "This Week") || (s === "Follow Up");
                    return isPastDue || isThisWeek;
                }}

                if (viewFilter === "next2") {{
                    return (w >= currentWeekNum && w <= currentWeekNum + 1) || (s === "This Week") || (s === "Follow Up");
                }}

                if (viewFilter === "next4") {{
                    return (w >= currentWeekNum && w <= currentWeekNum + 3) || (s === "This Week") || (s === "Follow Up");
                }}

                if (viewFilter === "overdue" || viewFilter === "late") {{
                    if (s === "N/A") return false;
                    if (contacted) return false;
                    return (w === -2) || (w >= 1 && w <= currentWeekNum) || (s === "This Week") || (s === "Follow Up");
                }}
            ...
            // Sort by group precedence first, then priority, then name
            ...
            filtered.sort((a, b) => {{
                ...
                const contactedA = a.lastTimeContacted && a.lastTimeContacted !== "" && a.lastTimeContacted.toLowerCase() !== "never";

                const contactedB = b.lastTimeContacted && b.lastTimeContacted !== "" && b.lastTimeContacted.toLowerCase() !== "never";

                function getSortGroup(c, s, w, contacted) {{
                    if (s === "N/A") return 4;
                    if (w === -2 || (w >= 1 && w < currentWeekNum && !contacted)) return 1;
                    if (w === currentWeekNum || s === "This Week" || s === "Follow Up") return 2;
                    return 3;
                }}

                const groupA = getSortGroup(a, sa, wa, contactedA);

                const groupB = getSortGroup(b, sb, wb, contactedB);

                if (groupA !== groupB) return groupA - groupB;

                if (groupA === 3 && wa !== wb) return wa - wb;
            ...
            filtered.forEach(c => {{
                ...
                if (s === "N/A") {{
                    ...
                    label = "Relationship Contacts (N/A)";

                }} else if (w === -2 || (w >= 1 && w < currentWeekNum && !contacted)) {{

                    label = "🚨 Past Due — Action Required";

                    dateLabel = "Requires updating to current or future week";

                }} else if (w === currentWeekNum || s === "This Week" || s === "Follow Up") {{

                    label = "📅 This Week";

                    const end = new Date(nowWS.getTime() + 6 * 86400000);

                    dateLabel = nowWS.toLocaleDateString("en-US", {{month:"short", day:"numeric"}}) + " - " + end.toLocaleDateString("en-US", {{month:"short", day:"numeric"}});
            ...
            Object.entries(groups).forEach(([label, group]) => {{
                ...
                const isPastDue = label.startsWith("🚨");

                const isNA = label.startsWith("Relationship");
                ...
                hdr.style.cssText = `display:flex;align-items:baseline;gap:10px;margin:18px 0 6px;padding-bottom:6px;border-bottom:2px solid ${{isPastDue ? "#ef4444" : isThisWeek ? "#2563eb" : isPast ? "#dc2626" : isNA ? "#9ca3af" : "#e5e7eb"}};`;

                hdr.innerHTML = `

                    <span style="font-size:15px;font-weight:700;color:${{isPastDue?"#ef4444":isThisWeek?"#1d4ed8":isPast?"#dc2626":isNA?"#9ca3af":"#1f2328"}}">${{label}}</span>
                ...
                group.items.forEach(c => {{
                    ...
                    if (isPastDue) tr.style.cssText += "border-left:3px solid #ef4444;";
                    ...
                    let deadlineCell = "N/A";

                    if (cDate) {{

                        const weeksUntil = weeksBetween(nowWS, getWeekStart(cDate));

                        deadlineCell = weeksUntil === 0 ? '<span style="color:#2563eb;font-weight:700">This Week</span>'

                            : weeksUntil < 0 ? '<span style="color:#ef4444;font-weight:700">Overdue</span>'

                            : `<span style="color:#374151">${{weeksUntil}}w left</span>`;

                    }} else if (s === "Past Due") {{

                        deadlineCell = '<span style="color:#ef4444;font-weight:700">Past Due</span>';

                    }} else if (s === "Follow Up") {{

                        deadlineCell = '<span style="color:#2563eb;font-weight:700">Follow Up</span>';
        ...
        async function syncEditsToGitHub() {{
            ...
            try {{
                ...
                localStorage.setItem(GH_KEY_PREFIX + "last_pushed_sha", newSha);

                statusEl.innerHTML = '<span style="color:#16a34a; font-weight:700;">✅ Success! Changes committed & pushed directly.<br><span style="font-size:11px; font-weight:normal; color:#475569;">Saved locally and pushed to GitHub. Clearing local cache as soon as GitHub Pages deploys in 30-60s...</span></span>';
        ...
        async function syncEditsToGitHubBackground() {{
            ...
            try {{
                ...
                showToast("Saved & Synced to GitHub! (Deploying in background...)", "success");
        ...
        async function checkGitHubForUpdates() {{
            ...
            try {{
                ...
                // Check if the deployed file has our last pushed changes
                const lastPushedSha = localStorage.getItem(GH_KEY_PREFIX + "last_pushed_sha");
                if (lastPushedSha && currentSha === lastPushedSha) {{
                    console.log("GitHub Pages / Repository has deployed our latest edits. Clearing temporary local storage edits.");
                    localStorage.removeItem(LS_KEY);
                    localStorage.removeItem(GH_KEY_PREFIX + "last_pushed_sha");
                }}
        ...
        function startRealTimeSync() {{
            ...
            checkGitHubForUpdates(); // Run immediately on startup
